...
"chatml_nr": """{%- set shared_prompt = __RULE__ -%}
{{- '<|im_start|>system\\n' + shared_prompt + '<|im_end|>\\n' -}}
{%- for message in messages -%}
    {{- '<|im_start|>' + message.role + '\\n' + message.content + '<|im_end|>\\n' -}}
{%- endfor -%}
...
{%- endfor -%}
{{- '<|im_start|>assistant\n' -}}
""".replace("__RULE__", repr(REASONING_SHARED_RPG_RULE)),
...
REASONING_QWEN_WARMUP = """{%- set shared_prompt = __RULE__ -%}
{{- '<|im_start|>system\n' + shared_prompt + '<|im_end|>\n' -}}
{%- for message in messages -%}
<|im_start|>{{ message.role }}
{{ message.content }}<|im_end|>
...
WARMUP_TYPES = [QWEN_WARMUP, REASONING_QWEN_WARMUP]
