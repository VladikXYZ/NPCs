...
class Benchmarker:
    def __init__(self, dev=None):
        ...
        if dev is None:
            ...
            if 0 <= dev < len(self.devices):
                ...
                sys.exit(f"Invalid device value:{dev}")
    ...
    def _process_messages(self, llm, index):
        model = self.models[index]
        row = [model] + ERROR_ROW
        failed = False
        model_log = []

        # Grab a fresh, independent copy of the chat history for this specific model run
        ...
        try:
            ...
            for user_input in tqdm(messages, desc=f"Testing {index + 1}/{len(self.models)} {model}", unit="prompt"):
    ...
    def _run_benchmark(self):
        ...
        for i, model in enumerate(self.models):
            ...
            if llm:
                # Call the isolated processing logic
                model_log = self._process_messages(llm, i)
    ...
    if len(sys.argv) == 2:
        ...
        if num == -1:
            ...
            for j in range(len(devices)):
                prev = time.time()
                subprocess.run([sys.executable, "bench_test.py", str(j)])
