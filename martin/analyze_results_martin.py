...
# The EXACT hardcoded order requested
ORDERED_MODEL_NAMES = [
    "Gemma 4 E2B",
    "Gemma 4 E4B",
    "Llama 3.1",
...
def plot_jsr_by_model(df):
    ...
    # Force reindex directly from the ORDERED_MODEL_NAMES array
    model_jsr_series = model_stats.reindex(ORDERED_MODEL_NAMES).fillna(0)
    
    fig, ax = plt.subplots(figsize=(18, 10), facecolor='none') 
    ax.set_facecolor('none') 
    
    barplot = sns.barplot(x=model_jsr_series.values, y=model_jsr_series.index, palette="flare", ax=ax)
    
    plt.title('Jailbreak Success Rate (JSR) by Model\n(Higher % = More Vulnerable)', fontsize=24, fontweight='bold', pad=25, color='white')
    plt.xlabel('Jailbreak Success Rate (%)', fontsize=20, labelpad=15, color='white')
    plt.ylabel('Model', fontsize=20, labelpad=15, color='white')
    plt.xlim(0, 100)
    ...
    for i, v in enumerate(model_jsr_series.values):
        ax.text(v + 1, i, f"{v:.1f}%", color='white', va='center', fontweight='bold', fontsize=18)
        
    plt.tight_layout()
    save_path = os.path.join(OUTPUT_DIR, 'jsr_by_model.png')
    plt.savefig(save_path, format='png', dpi=300, transparent=True)
    print(f"Saved custom ordered bar chart to: {save_path}")
    plt.close()
...
def plot_single_consolidated_heatmap(df):
    ...
    # Reindex the Matrix dataframe directly from the ORDERED_MODEL_NAMES array
    if existing_cols:
    ...
    fallback_fonts = ['SimHei', 'Microsoft YaHei', 'Arial Unicode MS', 'DejaVu Sans', 'sans-serif']
    ...
    plt.title('Master Heatmap: Comprehensive Vulnerability Matrix', fontsize=24, fontweight='bold', pad=25, color='white')
    plt.xlabel('Attack Category', fontsize=20, labelpad=15, color='white')
    plt.ylabel('Model', fontsize=20, labelpad=15, color='white')
    
    ax.tick_params(colors='white', which='both', labelsize=20) 
    
    font_prop = FontProperties(family=fallback_fonts, size=10)
    ...
    cbar.set_label('Jailbreak Success Rate (%)', color='white', labelpad=15, fontsize=20)
    ...
    plt.savefig(save_path, format='png', dpi=300, transparent=True)
    print(f"Saved ordered matrix chart to: {save_path}")
    plt.close()
...
def main():
    ...
    print("Regenerating both graphs locked to explicit mapping list...")
