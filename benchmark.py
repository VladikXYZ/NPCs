...
# --- 1. THE WORKER (Runs the actual C++ engine & Pandas log) ---
def run_worker(device_id, device_name, device_type, model_path):
...
# --- 2. THE MANAGER (Spawns the isolated workers based on your setup.py) ---
if __name__ == "__main__":

    # Check if this script was launched by the Manager with arguments
    if len(sys.argv) == 5:
        # We are the WORKER. Grab the arguments and run the test.
        target_id = sys.argv[1]
        target_name = sys.argv[2]
        target_type = sys.argv[3]
        target_model = sys.argv[4]
        run_worker(target_id, target_name, target_type, target_model)

    # If no arguments were passed, we are the MANAGER.
    else:
        print("🚀 Starting Benchmark Manager...")

        # Pull your devices dynamically from your setup.py (which reads devices.json)
        opt = Options()

        for device in opt.devices:
            # (Optional) Skip the CPU test if you don't want it chewing up your system RAM
            # if device["type"] == "CPU":
            #     continue

            for i in range(len(opt.models)):
                if i < 1:  # Testing the first 3 models
                    ...
                    # Spawn a completely isolated Python process for this specific test
                    subprocess.run([
                        sys.executable,  # Uses your current Python environment
                        __file__,  # Points to this exact script file
                        str(device["id"]),  # sys.argv[1]
                        str(device["name"]),  # sys.argv[2]
                        str(device["type"]),  # sys.argv[3]
                        model_path  # sys.argv[4]
                    ])

                    print(f"⏱️ It took: {time.time() - start:.2f} seconds")

                    # Give the OS and Vulkan driver 1.5 seconds to flush VRAM before the next test
                    print(f"🧹 [MANAGER] Test finished. Flushing VRAM...")
                    time.sleep(1.5)
